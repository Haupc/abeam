...
def reMap(elem):
    splitedKey = elem[0].split(':')
    print("negative: =>>>>>>>>>>>>>>", elem)
    return (splitedKey[2], splitedKey[0])
...
def main(argv=None, save_main_session=True):
    ...
    print("---------------------> args: {}".format(known_args.input))
    ...
    with beam.Pipeline(options=pipeline_options) as p:  # auto call p.run()
        ...
        recordPCol = (p | 'readFromAvro' >> ReadFromAvro(known_args.input)
                        ...
                        | 'filterBalance' >> beam.Filter(isNegative)
                        | 'reMap' >> beam.Map(reMap)
                        | 'count' >> beam.combiners.Count.PerKey()
                        | 'format' >> beam.Map(lambda x: "%s:%d" % (x[0], x[1]))

                      )
        recordPCol | 'writeToText' >> WriteToText(
            'gs://krystal-log-warehouse-v1/neg/%s.txt' % known_args.chain_id, num_shards=1)
        # './output-%s.txt' % known_args.chain_id, num_shards=1)
